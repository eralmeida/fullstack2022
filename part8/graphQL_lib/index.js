...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => books.filter((book) => (args.author ? book.author === args.author : true)).filter((book) => (args.genre ? book.genres.includes(args.genre) : true)),
    allAuthors: () =>
      authors.map((author) => {
  ...
  Mutation: {
    ...
    editAuthor: (root, args) => {
      let updatedAuthor = authors.find((author) => author.name === args.author)
      if (updatedAuthor) {
        updatedAuthor.born = args.setBornTo
        updatedAuthor = { ...updatedAuthor, bookCount: books.filter((book) => book.author === updatedAuthor.name).length }
      }
